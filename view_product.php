...
$getposts = mysqli_query($db,"SELECT * FROM products WHERE id ='$pid'") or die(mysqli_error());
...
<body>
	...
	<div class="categolis">
		<table>
			<tr>
				<th>
					<a href="hoody.php" style="text-decoration: none;color: #ddd;padding: 4px 12px;background-color: #c7587e;border-radius: 12px;">Hoody</a>
				</th>
				<th><a href="ornament.php" style="text-decoration: none;color: #ddd;padding: 4px 12px;background-color: #c7587e;border-radius: 12px;">Jeans</a></th>
				...
				<th><a href="hat.php" style="text-decoration: none;color: #ddd;padding: 4px 12px;background-color: #c7587e;border-radius: 12px;">Hat</a></th>
	...
	<div style="padding: 30px 95px; font-size: 25px; margin: 0 auto; display: table; width: 98%;">
		...
		<div>
		<?php 
			$getposts = mysqli_query($db,"SELECT * FROM products WHERE available >='1' AND id != '".$pid."' AND item ='".$item."'  ORDER BY RAND() LIMIT 3") or die(mysqli_error());
					if (mysqli_num_rows($getposts)) {
					echo '<ul id="recs">';
